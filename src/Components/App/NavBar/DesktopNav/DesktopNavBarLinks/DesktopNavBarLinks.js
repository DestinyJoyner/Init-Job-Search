...
function DesktopNavBarLinks({ path, label, clickfunction }) {
  ...
  const jobNavOptions = isRecruiterAcc ? [{value:"Browse Jobs", route:"/jobs"},{value: "Post New Job", route:"/jobs/new"}] : null
...
route:"/"}]
  ...
  return (
    <div 
    className={label === "Home"?` desktopNavBarLinks desktopNavBarLinks_${label}`: "desktopNavBarLinks"}
    ...
    >
      <span>{label} {navBarLinkDropdownValues[path] &&<BiChevronDown />}</span>
    </Link>
